...
from PyQt6.QtWidgets import (
    ...
    QGraphicsPathItem, QSlider, QLabel, QColorDialog, QFrame
)
from PyQt6.QtGui import (
    QAction, QIcon, QPen, QColor, QPainter, QPixmap, QPainterPath,
    QUndoStack, QUndoCommand # こちらに移動
)
from PyQt6.QtCore import Qt, QDir, QPointF, QRectF
...
class DrawingScene(QGraphicsScene):
    ...
    def erase_at(self, position):
        ...
        erase_rect = QRectF(position - QPointF(radius, radius), QPointF(radius*2, radius*2))
...
class MainWindow(QMainWindow):
    ...
    def create_actions_and_shortcuts(self):
        ...
        for key, method in {
            "B": self.pen_button.click, "E": self.eraser_button.click,
            "Ctrl+S": self.save_dataset_pair, "Left": self.show_prev_image,
            "Right": self.show_next_image
        }.items():
            action = QAction(self, shortcut=key, triggered=method)
    ...
    def open_folder_dialog(self):
        dir_path = QFileDialog.getExistingDirectory(self, "フォルダを選択", self.save_dir or QDir.homePath())
        if dir_path:
            self.image_files = []
    ...
    def show_next_image(self):
        if self.current_image_index < self.file_list_widget.count() - 1:
            self.file_list_widget.setCurrentRow(self.current_image_index + 1)
    ...
    def save_dataset_pair(self):
        ...
        if rect.isNull(): # 何も描かれていない場合は空の画像を作成
        ...
        # アイテムに進捗マークを付ける
        item = self.file_list_widget.item(self.current_image_index)
